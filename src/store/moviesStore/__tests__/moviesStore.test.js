import {FAVOURITES, getStoreMovie, resetStore, setStoreMovie, store} from "../moviesStore";
...
describe('working with store', () => {
    test('set movie', () => {
        setStoreMovie(FAVOURITES, movies[0]);
        expect(store.FAVOURITES[movies[0].id]).toBe(movies[0]);
    });
    test('get movie', () => {
        const movie = getStoreMovie(FAVOURITES, movies[0]);
    ...
    test('set multiple movies', () => {
        setStoreMovie(FAVOURITES, movies[0]);
        setStoreMovie(FAVOURITES, movies[1]);
        expect(Object.keys(store.FAVOURITES).length).toBe(2);
    });
})